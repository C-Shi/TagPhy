...
export function TagCard({ tag }: { tag: TagResponse }) {
  return (
    <Link
    ...
    >
      <div className="truncate text-lg font-semibold text-ink">{tag.name}</div>
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        <span className="rounded px-2 py-0.5 text-sm font-medium text-label-pink ring-1 ring-label-pink/25">
          {tag.children.length}{" "}
          {tag.children.length === 1 ? "child tag" : "child tags"}
        </span>
      </div>
    </Link>
  );
